...
class SemanticAnalyzer:
    ...
    def visit_VariableDeclaration(self, node):
        ...
        if (node.data_type == 'list' or node.data_type == 'tup'):
            ...
            if (node.equal_to):
                ...
                print(test_lst)
                print("bsdfhdsgbsds")
                print("type_ckeck:",type(test_lst[0])) if test_lst else print("test_lst is empty")
            ...
            #to handle num a = b[2];
            if node.equal_to:
                if len(node.equal_to) == 1:
                    if node.equal_to[0].variable_name is not None:
                        mutability, type_name = node.equal_to[0].data_type.split()
                        self.declare_variable(data_type=type_name, mutability=mutability, name=node.equal_to[0].variable_name)
                elif node.equal_to[1]:
                    if node.equal_to[1].identifier is not None:
                        if node.equal_to[1].expression:
                            if node.equal_to[1].expression.terms[0].value is not None:
    ...
    def visit_Assignment(self, node):
        ...
        if len(var_info) > 2:
            ...
            #to handle a = b[2];
            elif (node.value.terms[0].identifier is not None and node.value.terms[0].expression is not None and node.value.terms[0].expression.terms[0].value is not None) :
